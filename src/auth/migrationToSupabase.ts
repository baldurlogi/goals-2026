...
export async function migrateLocalStorageToSupabase() {
  ...
  if (log) {
    ...
    await upsert("nutrition_logs", {
      user_id: uid, log_date: p.date,
      eaten: p.eaten ?? {}, custom_entries: p.customEntries ?? [],
    }, "user_id,log_date");
  }
  ...
  if (saved) {
    for (const m of JSON.parse(saved)) {
      await upsert("saved_meals", { id: m.id, user_id: uid, name: m.name, macros: m.macros, emoji: m.emoji }, "id");
    }
  }
  ...
  if (sched) {
    ...
    await upsert("schedule_logs", {
      user_id: uid, log_date: p.date, view: p.view ?? "wfh", completed: p.completed ?? [],
    }, "user_id,log_date");
  }
  ...
  if (todos) {
    for (const t of JSON.parse(todos)) {
      await upsert("todos", {
        id: t.id, user_id: uid, text: t.text, done: t.done,
  ...
  if (fitness) {
    ...
    for (const [liftId, r] of Object.entries(p.lifts ?? {})) {
      ...
      await upsert("fitness_lifts", { user_id: uid, lift_id: liftId, goal: rec.goal, history: rec.history }, "user_id,lift_id");
    }
    for (const [skillId, r] of Object.entries(p.skills ?? {})) {
      const rec = r as { goal: number; goal_label?: string; history: object[] };
      await upsert("fitness_skills", { user_id: uid, skill_id: skillId, goal: rec.goal, goal_label: rec.goal_label, history: rec.history }, "user_id,skill_id");
    }
  }

  // Goal progress
  const goals = localStorage.getItem("goals_v1");
  if (goals) {
    const p = JSON.parse(goals);
    for (const [goalId, done] of Object.entries(p.done ?? {})) {
      await upsert("goal_progress", { user_id: uid, goal_id: goalId, done }, "user_id,goal_id");
    }
  }
  ...
  if (errors.length) {
    ...
    console.log("✅ Migration complete — all localStorage data moved to Supabase");
  }
}