...
function loadLocalState(goalId: string): GoalReadingState {
  return {
    streak: safeParse(
      localStorage.getItem(ns(goalId, 'streak')),
      defaultStreak(),
    ),
    minutes: safeParse(
      localStorage.getItem(ns(goalId, 'minutes')),
      defaultMinutes(),
    ),
...
function normalizeGoalState(
  raw?: Partial<GoalReadingState> | null,
): GoalReadingState {
...
async function saveGoalState(
  ...
  const { error: upsertError } = await supabase
    .from('reading_state')
    .upsert(
      { user_id: uid, state: merged, updated_at: getLocalDateKey() },
      { onConflict: 'user_id' },
    );
...
export async function getReadingStreak(
  goalId: string,
): Promise<ReadingStreakState> {
  const state = await getGoalState(goalId);
  return state.streak;
}

export async function setReadingStreak(
  goalId: string,
  next: ReadingStreakState,
): Promise<void> {
...
// ---- minutes ----
export async function getReadingMinutes(
  goalId: string,
): Promise<ReadingMinutesState> {
  const state = await getGoalState(goalId);
  return state.minutes;
}

export async function setReadingMinutes(
  goalId: string,
  next: ReadingMinutesState,
): Promise<void> {
  const cur = await getGoalState(goalId);
  await saveGoalState(goalId, {
    ...cur,
    minutes: normalizeGoalState({ minutes: next }).minutes,
  });
}
