          ...
          <label className="flex items-start gap-3 cursor-pointer group">
            <div className="mt-0.5 shrink-0">
              <input
                type="checkbox"
                checked={agreed}
                onChange={e => { setAgreed(e.target.checked); if (error) setError(null); }}
                className="sr-only"
              />
              <div
                className={cn(
            ...
            <span className="text-xs text-muted-foreground leading-relaxed">
              I agree to Kairo&apos;s{" "}
              <Link to="/terms" className="text-violet-400 hover:underline" onClick={e => e.stopPropagation()}>
                Terms of Service
              </Link>{" "}
              and{" "}
              <Link to="/privacy" className="text-violet-400 hover:underline" onClick={e => e.stopPropagation()}>
                Privacy Policy
              </Link>
