...
export default function DashboardPage() {
  ...
  // Quick actions only shown if the module is enabled
  const quickActions = [
    has("nutrition") && { icon: <Apple className="h-4 w-4" />,      label: "Log a meal",   sub: "Nutrition tab",  href: "/app/nutrition", color: "bg-orange-500/10 text-orange-600 dark:text-orange-400" },
    has("reading")   && { icon: <BookOpen className="h-4 w-4" />,   label: "Update pages", sub: "Reading tab",    href: "/app/reading",   color: "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" },
    has("fitness")   && { icon: <Dumbbell className="h-4 w-4" />,   label: "Log a PR",     sub: "Fitness",        href: "/app/fitness",   color: "bg-violet-500/10 text-violet-600 dark:text-violet-400" },
    has("goals")     && { icon: <TrendingUp className="h-4 w-4" />, label: "Review goals", sub: "Upcoming tasks", href: "/app/upcoming",  color: "bg-rose-500/10 text-rose-600 dark:text-rose-400" },
  ].filter(Boolean) as { icon: React.ReactNode; label: string; sub: string; href: string; color: string }[];
    ...
    <div className="space-y-5">
      ...
      {modules.size === 0 && (
        <div className="rounded-2xl border border-dashed p-12 text-center space-y-3">
          ...
          <p className="text-sm text-muted-foreground">Go to Profile settings to choose what you want to track.</p>
