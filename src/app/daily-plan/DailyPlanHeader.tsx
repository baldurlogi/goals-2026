...
export function DailyPlanHeader() {
  ...
  const dailyItems  = enabledDefs.filter((m) => m.section === "Daily Plan").map((m) => ({ label: m.navLabel, href: m.href }));
  const goalsItems: NavItem[] = modules.has("goals")
    ? [{ label: "🎯 All Goals", href: "/app/goals" }, { label: "📌 Upcoming", href: "/app/upcoming" }]
    : [];
  const otherItems  = enabledDefs.filter((m) => m.section === "Other").map((m) => ({ label: m.navLabel, href: m.href }));
  ...
  const currentItem = sections.flatMap((s) => s.items).find((i) => pathname.startsWith(i.href));
              ...
              <div key={section.label}>
                ...
                {section.items.map((item) => (
                  <DropdownMenuItem key={item.href} asChild>
                    <Link
                      to={item.href}
                      className={cn("cursor-pointer", pathname.startsWith(item.href) && "font-semibold")}
                    >
                      {item.label}
                    </Link>
                  </DropdownMenuItem>
